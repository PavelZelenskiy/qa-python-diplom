...
def test_remove_ingredient(prepared_burger):
...
def test_get_price(prepared_burger, bun, ingredient):
    expected_price = bun.get_price() * 2 + ingredient.get_price()
    assert prepared_burger.get_price() == expected_price


def test_get_receipt(prepared_burger, bun, ingredient):
